...
function SquareUI({
  ...
  return (
    <Droppable droppableId={`${String.fromCharCode(65 + column)}${row + 1}`}>
      {(provided, snapshot) => (
        <div
          onMouseDown={() => piece && getValidMoves.mutate({ row, column })}
          onMouseUp={() => piece && clearSuggestions.mutate()}
          ref={provided.innerRef}
          className={`h-20 w-20 ${
            isLightSquared ? "bg-slate-300" : "bg-green-600"
          } ${possibleMove ? "border-4 border-solid border-slate-600" : ""} ${
        ...
        >
          {piece && (
            <Draggable
              draggableId={`${String.fromCharCode(65 + column)}${row + 1}_${
                piece.name
              }`}
              index={0}
            >
              {(provided, snapshot) => (
                <Image
                  ...
                  src={`/images/${piece.color ? "white" : "black"}_${
                    piece.name
                  }.png`}
                  alt=""
                />
